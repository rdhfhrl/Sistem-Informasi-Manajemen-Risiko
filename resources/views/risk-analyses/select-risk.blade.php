...
@section('title', 'Pilih Risiko untuk Analisis - SIMR')
            ...
            <div class="p-5">
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-blue-700 text-sm">
                                Pilih risiko dari daftar di bawah ini untuk melihat atau membuat analisis risiko.
                                Anda juga bisa melihat semua analisis tanpa filter.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <a href="{{ route('risk-analyses.all') }}" class="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-black p-6 rounded-lg shadow-md transition-all duration-300 transform hover:-translate-y-1">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <div class="text-lg font-bold">Semua Analisis</div>
                                    <div class="text-sm opacity-90 mt-1">Lihat semua analisis risiko</div>
                                </div>
                            </div>
                        </a>
                        ...
                        <a href="{{ route('risks.index') }}" class="bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-black p-6 rounded-lg shadow-md transition-all duration-300 transform hover:-translate-y-1">
                            <div class="flex items-center">
                                <div class="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center mr-4">
                ...
                <div class="mt-8">
                    ...
                    @php
                        // Ambil 10 risiko terbaru
                        $risks = \App\Models\Risk::with(['analyses' => function($query) {
                                ...
                                <div class="bg-white border rounded-lg p-4 hover:shadow-lg transition-shadow duration-300">
                                    <div class="flex justify-between items-start mb-3">
                                        ...
                                        @if($risk->analyses->count() > 0)
                                            ...
                                            <span class="px-2 py-1 rounded-full text-xs font-medium 
                                                @if($latestAnalysis->risk_level == 'sangat_rendah') bg-green-100 text-green-800
                                                @elseif($latestAnalysis->risk_level == 'rendah') bg-blue-100 text-blue-800
                                                @elseif($latestAnalysis->risk_level == 'sedang') bg-yellow-100 text-yellow-800
                                                @elseif($latestAnalysis->risk_level == 'tinggi') bg-orange-100 text-orange-800
                                    ...
                                    <div class="flex justify-between items-center mt-4">
                                        <div class="text-xs text-gray-500">
                                            @if($risk->analyses->count() > 0)
                                                Analisis terakhir: {{ \Carbon\Carbon::parse($risk->analyses->first()->analysis_date)->format('d/m/Y') }}
                                            @else
                                                Belum ada analisis
                                            @endif
                                        </div>
                                        <div class="flex space-x-2">
                                            <a href="{{ route('risk-analyses.by-risk', $risk->risk_id) }}" 
                                               class="text-xs bg-blue-100 text-blue-800 hover:bg-blue-200 px-3 py-1 rounded-full transition-colors duration-300 flex items-center">
                                                <i data-feather="eye" class="w-3 h-3 mr-1"></i> Lihat Analisis
                                            </a>
                                            <a href="{{ route('risk-analyses.create', $risk->risk_id) }}" 
                                               class="text-xs bg-green-100 text-green-800 hover:bg-green-200 px-3 py-1 rounded-full transition-colors duration-300 flex items-center">
                                                <i data-feather="plus" class="w-3 h-3 mr-1"></i> Analisis Baru
                                            </a>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @else
                        <div class="text-center py-12 bg-gray-50 rounded-lg">
                            ...
                            <p class="text-gray-500 mb-6">Tambahkan risiko terlebih dahulu untuk melakukan analisis</p>
