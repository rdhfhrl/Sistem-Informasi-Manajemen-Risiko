...
@section('title', 'Edit Evaluasi Risiko - SIMR')

@section('page-title', 'Edit Evaluasi Risiko')
        ...
        <div class="intro-y box">
            ...
            <div class="p-5">
                ...
                <div class="mb-8 bg-gray-50 p-6 rounded-lg">
                    <h4 class="font-medium text-gray-700 mb-4">Ringkasan Evaluasi Saat Ini</h4>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="text-center">
                            <div class="text-gray-600 text-sm mb-2">Tanggal Evaluasi</div>
                            <div class="font-medium text-lg">{{ $evaluation->evaluation_date->format('d F Y') }}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-gray-600 text-sm mb-2">Prioritas Penanganan</div>
                ...
                <form action="{{ route('risk-evaluations.update', [$risk->risk_id, $evaluation->risk_evaluation_id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                                    ...
                                    @foreach(['sangat tinggi', 'tinggi', 'sedang', 'rendah'] as $priority)
                                        <div class="flex items-center mb-3">
                                            <input type="radio" 
                                                   ...
                                                   {{ old('risk_evaluation_priority', $evaluation->risk_evaluation_priority) == $priority ? 'checked' : '' }}
                                                   required>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="relative">
                                    <input type="number" 
                                           ...
                                           value="{{ old('projected_risk_score', $evaluation->projected_risk_score) }}"
                                           min="1" 
                                           max="25" 
                                           step="0.01"
                        ...
                        <div>
                            ...
                            <div class="mb-6">
                                ...
                                <textarea id="mitigation_decision" 
                                          ...
                                          required>{{ old('mitigation_decision', $evaluation->mitigation_decision) }}</textarea>
                                <div class="text-gray-500 text-xs mt-1">
                    ...
                    <div class="mt-8 mb-6 bg-gray-50 p-6 rounded-lg">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div class="text-center">
                                <div class="text-gray-600 text-sm mb-2">Proyeksi Skor Baru</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const oldProjectedScore = {{ $evaluation->projected_risk_score ?? 0 }};
    ...
    function updateScorePreview() {
        ...
        if (projectedScore && !isNaN(projectedScore)) {
            ...
            // Calculate change from current score
            if (currentScore > 0) {
                const changeFromCurrent = projectedScore - currentScore;
                const changeFromOld = projectedScore - oldProjectedScore;
                
                if (changeFromCurrent !== 0) {
                    const percentageChange = (changeFromCurrent / currentScore) * 100;
                    
                    if (changeFromCurrent > 0) {
                        scoreChangeIndicator.innerHTML = `<span class="text-red-600">Meningkat ${changeFromCurrent.toFixed(2)} poin</span>`;
                        ...
                        scoreChangeIndicator.innerHTML = `<span class="text-green-600">Menurun ${Math.abs(changeFromCurrent).toFixed(2)} poin</span>`;
                    ...
                    if (oldProjectedScore > 0 && changeFromOld !== 0) {
                        const changeText = changeFromOld > 0 ? 'meningkat' : 'menurun';
                        const changeAmount = Math.abs(changeFromOld).toFixed(2);
                        scoreChangeDetails.textContent += ` | Dari ${oldProjectedScore} ${changeText} ${changeAmount} poin`;
                    }
                } else {
