...
@section('title', 'Edit Identifikasi Risiko - SIMR')

@section('page-title', 'Edit Identifikasi Risiko: ' . $risk->risk_code)
...
<div class="grid grid-cols-12 gap-6">
    <div class="col-span-12 xl:col-span-8">
        <!-- Form Edit Identifikasi Risiko -->
        <div class="intro-y box">
            <div class="flex items-center p-5 border-b border-gray-200">
                <div class="w-10 h-10 rounded-full bg-yellow-100 flex items-center justify-center mr-3">
                    <i data-feather="edit" class="w-5 h-5 text-yellow-600"></i>
                </div>
            ...
            <form method="POST" action="{{ route('risk-identifications.update') }}" class="p-5">
                @csrf
                @method('PUT')
                    ...
                    <div class="p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-200">
                            <div class="text-xs text-gray-500">
                                ...
                                Identifikasi dibuat: {{ $identification->created_at->format('d F Y H:i') }}
                            </div>
                            <div class="text-xs text-gray-500 mt-1">
                                <i data-feather="edit" class="w-3 h-3 inline mr-1"></i>
                                Terakhir diperbarui: {{ $identification->updated_at->format('d F Y H:i') }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4 bg-gray-50 rounded-lg mb-6">
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-200">
                        <div class="text-sm text-gray-600">
                            ...
                            Data identifikasi akan diperbarui setelah disimpan
                        </div>
                    </div>
                </div>
    ...
    <div class="col-span-12 xl:col-span-4">
        ...
        <div class="intro-y box">
            <div class="flex items-center p-5 border-b border-gray-200">
                <h2 class="font-medium text-base mr-auto">
                    <i data-feather="edit-2" class="w-5 h-5 mr-2"></i> Panduan Edit
                </h2>
            </div>
            <div class="p-5">
                <div class="space-y-4">
                    <div class="flex items-start">
                        ...
                        <div>
                            <h4 class="font-medium">Perubahan yang Akurat</h4>
                            <p class="text-sm text-gray-600">Pastikan perubahan sesuai dengan perkembangan risiko</p>
                        </div>
                    </div>
                    <div class="flex items-start">
                        ...
                        <div>
                            <h4 class="font-medium">Update Berkala</h4>
                            <p class="text-sm text-gray-600">Identifikasi perlu diperbarui sesuai perubahan kondisi</p>
                        </div>
                    </div>
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Pastikan perubahan telah divalidasi dengan data terkini</p>
                        </div>
                    </div>
                    <div class="flex items-start">
                        ...
                        <div>
                            <h4 class="font-medium">Hapus Data</h4>
                            <p class="text-sm text-gray-600">Hapus hanya jika identifikasi sudah tidak relevan</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Info Identifikasi Saat Ini -->
        <div class="intro-y box mt-6">
            <div class="flex items-center p-5 border-b border-gray-200">
                <h2 class="font-medium text-base mr-auto">
                    <i data-feather="info" class="w-5 h-5 mr-2"></i> Data Saat Ini
                </h2>
            </div>
            <div class="p-5">
                <div class="space-y-3">
                    <div>
                        <div class="text-xs text-gray-500 mb-1">Jenis Kerugian:</div>
                        @if($identification->loss_type)
                            <div class="px-3 py-2 bg-red-50 rounded">
                                <div class="flex items-center">
                                    <i data-feather="{{ getLossTypeIcon($identification->loss_type) }}" class="w-4 h-4 text-red-600 mr-2"></i>
                                    <span class="font-medium text-red-700">{{ $identification->loss_type }}</span>
                                </div>
                            </div>
                        @else
                            <div class="px-3 py-2 bg-gray-50 rounded text-gray-500">
                                <i data-feather="slash" class="w-4 h-4 inline mr-1"></i> Belum diisi
                            </div>
                        @endif
                    </div>
                    
                    <div>
                        <div class="text-xs text-gray-500 mb-1">Jenis Pelanggaran:</div>
                        @if($identification->violation_type)
                            <div class="px-3 py-2 bg-blue-50 rounded">
                                <div class="flex items-center">
                                    <i data-feather="{{ getViolationTypeIcon($identification->violation_type) }}" class="w-4 h-4 text-blue-600 mr-2"></i>
                                    <span class="font-medium text-blue-700">{{ $identification->violation_type }}</span>
                                </div>
                            </div>
                        @else
                            <div class="px-3 py-2 bg-gray-50 rounded text-gray-500">
                    ...
                    <div>
                        <div class="text-xs text-gray-500 mb-1">Jenis Kegagalan:</div>
                        @if($identification->failure_type)
                            <div class="px-3 py-2 bg-teal-50 rounded">
                                <div class="flex items-center">
                                    <i data-feather="{{ getFailureTypeIcon($identification->failure_type) }}" class="w-4 h-4 text-teal-600 mr-2"></i>
                                    <span class="font-medium text-teal-700">{{ $identification->failure_type }}</span>
                                </div>
                            </div>
                        @else
                            <div class="px-3 py-2 bg-gray-50 rounded text-gray-500">
                    ...
                    <div>
                        <div class="text-xs text-gray-500 mb-1">Jenis Kesalahan:</div>
                        @if($identification->error_type)
                            <div class="px-3 py-2 bg-amber-50 rounded">
                                <div class="flex items-center">
                                    <i data-feather="{{ getErrorTypeIcon($identification->error_type) }}" class="w-4 h-4 text-amber-600 mr-2"></i>
                                    <span class="font-medium text-amber-700">{{ $identification->error_type }}</span>
                                </div>
                            </div>
                        @else
                            <div class="px-3 py-2 bg-gray-50 rounded text-gray-500">
                    ...
                    <div class="pt-3 border-t border-gray-200">
                        <div class="text-xs text-gray-500 mb-1">Riwayat:</div>
                        <div class="text-xs text-gray-600">
                            <i data-feather="calendar" class="w-3 h-3 inline mr-1"></i>
                            Dibuat: {{ $identification->created_at->format('d/m/Y H:i') }}
                        </div>
                        <div class="text-xs text-gray-600 mt-1">
                            <i data-feather="edit" class="w-3 h-3 inline mr-1"></i>
                            Update: {{ $identification->updated_at->format('d/m/Y H:i') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal__content">
        <div class="p-5">
            <div class="text-center">
                ...
                <h2 class="text-lg font-medium mb-2">Hapus Identifikasi Risiko</h2>
                <p class="text-gray-600 mb-6">
                    Apakah Anda yakin ingin menghapus identifikasi risiko ini?
                    <br>
                    <span class="text-sm text-gray-500">Aksi ini tidak dapat dibatalkan</span>
                </p>
            </div>
            
            <form id="delete-form" method="POST" action="{{ route('risk-identifications.destroy') }}">
                @csrf
                @method('DELETE')
                <input type="hidden" name="risk_id" value="{{ $risk->risk_id }}">
